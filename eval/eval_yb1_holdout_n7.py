"""eval_yb1_holdout_n7.py — extend YB1 hold-out eval to include 0516 plate-clone BAMs.

Compares ckpt prediction vs measured on:
  - Original 5 YB1 BAMs (paper v2.54 set)
  - 2 new 0516 plate-clone YB1 BAMs (n=7 total)
  - Per-group means + combined mean
...
def main():
    ...
    for i, bam in enumerate(ALL_7):
        ...
        if r is None:
            print(f"  {bam}: skipped (insufficient obs)")
            continue
        cos, mse, n = r
    ...
    print()
    if orig_cos:
        m = statistics.mean(orig_cos)
        s = statistics.stdev(orig_cos) if len(orig_cos) > 1 else 0
        print(f"  Original 5 BAMs mean = {m:+.4f} ± {s:.4f}  (n=5)")
    if new_cos:
        m = statistics.mean(new_cos)
        s = statistics.stdev(new_cos) if len(new_cos) > 1 else 0
        print(f"  0516 plate-clone (n=2) = {m:+.4f} ± {s:.4f}")
    if all_cos:
        m = statistics.mean(all_cos)
        s = statistics.stdev(all_cos) if len(all_cos) > 1 else 0
        print(f"  Combined (n=7)         = {m:+.4f} ± {s:.4f}")

    # Save
    out = Path(args.ckpt).parent / "yb1_holdout_n7_eval.json"
